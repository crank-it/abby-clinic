...
export async function TrackingScripts() {
  ...
  return (
    <>
      ...
      {/* Google Tag Manager noscript fallback (script is in head via layout.tsx) */}
      {config.gtm?.enabled && config.gtm.containerId && (
        <noscript dangerouslySetInnerHTML={{
          __html: `<iframe src="https://www.googletagmanager.com/ns.html?id=${config.gtm.containerId}" height="0" width="0" style="display:none;visibility:hidden"></iframe>`
        }} />
      )}
    </>
  );
